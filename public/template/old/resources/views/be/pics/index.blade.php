                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Person in Charge List</h1>
                            ...
                            <div class="col-12 col-md-5 d-flex align-items-start justify-content-end">{{-- Top Buttons Start --}}
                                ...
                                <button type="button" class="btn btn-outline-primary btn-icon btn-icon-start w-100 w-md-auto add-datatable" onclick="window.location.href='{{ route('pics.create') }}';">
                                    ...
                                    <table
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-muted text-small text-uppercase">Verify</th>
                                        ...
                                        <tbody>
                                            @forelse ($datas as $data)
                                                <tr>
                                                    ...
                                                    <td class="text-alternate">{{ $data->gender->name_len }}</td>
                                                    <td class="text-alternate">{{ $data->email }}</td>
                                                    <td class="text-alternate">{{ $data->province_id }} - {{ $data->province->name }}</td>
                                                    <td class="text-alternate">{{ $data->regency_id }} - {{ $data->regency->name }}</td>
                                                    <td class="text-alternate">{!! $data->verification_id == "1" ? '<span class="badge bg-success"> Yes </span>' : "" !!}</td>
                                                    <td class="text-alternate">
                                                        <form action="{{ route('pics.destroy', $data->id) }}" method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                                <a href="{{ route('pics.show', $data->id) }}" class="btn btn-warning btn-sm"><i data-acorn-icon="eye"data-acorn-size="13"></i> Show</a>
                                                            @can('master-edit')
                                                                <a href="{{ route('pics.edit', $data->id) }}" class="btn btn-primary btn-sm"><i data-acorn-icon="edit" data-acorn-size="13"></i> Edit</a>
                                                            @endcan
                                                            {{-- @can('master-delete')
