                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Regency Lists</h1>
                                <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                                <ul class="breadcrumb pt-0">
                                    <li class="breadcrumb-item"><a href="javascript:void(0);">Home</a></li>
                                    <li class="breadcrumb-item"><a href="javascript:void(0);">Regencies</a></li>
                                    ...
                                    <table
                                        ...
                                        <tbody>
                                            @forelse ($regencies as $regency)
                                                <tr>
                                                    <td>{{ $regency->id }}</td>
                                                    <td class="text-alternate">{{ $regency->name }}</td>
                                                    <td class="text-alternate">{{ $regency->province_id }}</td>
                                                    <td class="text-alternate">
                                                        <form action="{{ route('permissions.destroy', $regency->id) }}" method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            {{-- <a href="{{ route('regencies.show', $regency->id) }}" class="btn btn-warning btn-sm"><i class="bi bi-eye"></i> Show</a>--}}
                                                            @can('master-edit')
                                                                <a href="{{ route('regencies.edit', $regency->id) }}" class="btn btn-primary btn-sm"><i data-acorn-icon="edit" data-acorn-size="13"></i> Edit</a>
                                                            @endcan
                                                            @can('master-delete')
                                                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Do you want to delete this regenciy?');"><i data-acorn-icon="bin" data-acorn-size="13"></i> Delete</button>
                                                            @endcan
                                                        </form>
                                                    </td>
                                                </tr>
