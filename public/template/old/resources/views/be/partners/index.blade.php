                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Partner List</h1>
                            ...
                            <div class="col-12 col-md-5 d-flex align-items-start justify-content-end">{{-- Top Buttons Start --}}
                                ...
                                <button type="button" class="btn btn-outline-primary btn-icon btn-icon-start w-100 w-md-auto add-datatable" onclick="window.location.href='{{ route('partners.create') }}';">
                                    ...
                                    <table
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-muted text-small text-uppercase">Website</th>
                                        ...
                                        <tbody>
                                            @forelse ($datas as $data)
                                                <tr>
                                                    ...
                                                    <td class="text-alternate">{{-- $data->logo --}}
                                                        <div class="form-text"><img src="/imgpartners/{{ $data->logo <> '' ? $data->logo : 'empty.png' }}" class="mt-2 img-fluid rounded img-fluid rounded mb-1 me-1 sw-35" alt="logo image" style="max-width:100px;width:100%"/></div>
                                                    </td>
                                                    <td class="text-alternate">{{ $data->website }}</td>
                                                    <td class="text-alternate">{!! $data->f_active == "1" ? '<span class="badge bg-success"> Yes </span>' : "" !!}</td>
                                                    <td class="text-alternate">
                                                        {{-- <form action="{{ route('partners.destroy', $data->id) }}" method="post"> --}}
                                                            @csrf
                                                            @method('DELETE')
                                                                {{-- <a href="{{ route('partners.show', $data->id) }}" class="btn btn-warning btn-sm"><i class="bi bi-eye"></i> Show</a>--}}
                                                            @can('master-edit')
                                                                <a href="{{ route('partners.edit', $data->id) }}" class="btn btn-primary btn-sm"><i data-acorn-icon="edit" data-acorn-size="13"></i> Edit</a>
                                                            @endcan
                                                            {{-- @can('master-delete')
                                                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Do you want to delete this data?');"><i data-acorn-icon="bin" data-acorn-size="13"></i> Delete</button>
                                                            @endcan --}}
                                                        {{-- </form> --}}
                                                    </td>
                                                </tr>
                                            @empty
                                                <td colspan="3">
                                                    <span class="text-danger">
                                                        <strong>No Partner Found!</strong>
                                                    </span>
                                                </td>
                                            @endforelse
                                        </tbody>
