                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Turnover List</h1>
                                <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                                <ul class="breadcrumb pt-0">
                                    <li class="breadcrumb-item"><a href="javascript:void(0);">Home</a></li>
                                    <li class="breadcrumb-item"><a href="javascript:void(0);">Turnover</a></li>
                            ...
                            <div class="col-12 col-md-5 d-flex align-items-start justify-content-end">{{-- Top Buttons Start --}}
                                ...
                                <button type="button" class="btn btn-outline-primary btn-icon btn-icon-start w-100 w-md-auto add-datatable" onclick="window.location.href='{{ route('tovercats.create') }}';">
                                    ...
                                    <table
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-muted text-small text-uppercase">Category (ID)</th>
                                                <th class="text-muted text-small text-uppercase">Name (EN)</th>
                                                <th class="text-muted text-small text-uppercase">Category (EN)</th>
                                        ...
                                        <tbody>
                                            @forelse ($datas as $data)
                                                <tr>
                                                    <td>{{ $data->id }}</td>
                                                    <td class="text-alternate">{{ $data->name_lid }}</td>
                                                    <td class="text-alternate">{{ $data->cat_lid }}</td>
                                                    <td class="text-alternate">{{ $data->name_len }}</td>
                                                    <td class="text-alternate">{{ $data->cat_len }}</td>
                                                    <td class="text-alternate">{!! $data->f_active == "1" ? '<span class="badge bg-success"> Yes </span>' : "" !!}</td>
                                                    <td class="text-alternate">
                                                        <form action="{{ route('tovercats.destroy', $data->id) }}" method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                                {{-- <a href="{{ route('tovercats.show', $data->id) }}" class="btn btn-warning btn-sm"><i class="bi bi-eye"></i> Show</a>--}}
                                                            @can('master-edit')
                                                                <a href="{{ route('tovercats.edit', $data->id) }}" class="btn btn-primary btn-sm">Edit</a>
                                                            @endcan
                                                            {{-- @can('master-delete') --}}
                                            ...
                                            @empty
                                                <td colspan="3">
                                                    <span class="text-danger">
                                                        <strong>No Turnover Found!</strong>
                                                    </span>
                                                </td>
                                            @endforelse
                                        </tbody>
