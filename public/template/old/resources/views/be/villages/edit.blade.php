                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Edit Village</h1>
                        ...
                        <section class="scroll-section" id="basic">
                            ...
                            <div class="card mb-5">
                                ...
                                {{-- tooltip-label-end inputs should be wrapped in form-group class --}}
                                    <form id="frmPermission" class="tooltip-label-end" action="{{ route('villages.update', $village->id) }}" method="post">
                                        @csrf
                                        @method("PUT")
                                        <div class="mb-3 position-relative form-group w-100">
                                            ...
                                            <select id="select2Villages" name="district_id" required>
                                                ...
                                                @foreach($districts as $district)
                                                    <option value="{{$district->id}}" {{ ( $district->id == $village->district_id) ? 'selected' : '' }}>{{$district->id}} - {{$district->name}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="mb-3 position-relative form-group">
                                            ...
                                            <input type="text" class="form-control" name="id" value="{{ $village->id }}" required />
                                            <div class="form-text">Input <strong>9 characters numeric.</strong></div>
                                            @error('id')<div class="form-text">{{ $message }}</div>@enderror
                                        </div>
                                        <div class="mb-3 position-relative form-group">
                                            <label class="form-label">Name*</label>
                                            <input type="text" class="form-control" name="name" value="{{ $village->name }}" required />
                                        ...
                                        <div class="mb-3 position-relative form-group">
                                            ...
                                            <input type="text" class="form-control" name="nameupper" value="{{ $village->nameupper }}" required />
                                            <div class="form-text">Input <strong>minimum 5 characters.</strong></div>
                                            @error('nameupper')<div class="form-text">{{ $message }}</div>@enderror
                                        </div>
                                        <button type="submit" class="btn btn-icon btn-icon-end btn-primary"> <span>Update Village</span><i data-acorn-icon="chevron-right"></i></button>
                                    </form>
                                </div>
                            </div>
                        </section>
