                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Gender List</h1>
                                <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                                <ul class="breadcrumb pt-0">
                                    <li class="breadcrumb-item"><a href="javascript:void(0);">Home</a></li>
                                    <li class="breadcrumb-item"><a href="javascript:void(0);">Gender</a></li>
                                    ...
                                    <table
                                        ...
                                        <tbody>
                                            @forelse ($genders as $gender)
                                                <tr>
                                                    <td>{{ $gender->id }}</td>
                                                    <td class="text-alternate">{{ $gender->name_lid }}</td>
                                                    <td class="text-alternate">{{ $gender->name_len }}</td>
                                                    <td class="text-alternate">{!! $gender->f_active == "1" ? '<span class="badge bg-success"> Yes </span>' : "" !!}</td>
                                                    {{-- <td class="text-alternate"> --}}
                                                        {{-- <form action="{{ route('genders.destroy', $gender->id) }}" method="post"> --}}
                                                            {{-- @csrf --}}
                                                            {{-- @method('DELETE') --}}
                                                            {{-- <a href="{{ route('genders.show', $gender->id) }}" class="btn btn-warning btn-sm"><i class="bi bi-eye"></i> Show</a>--}}
                                                            {{-- @can('master-edit')
                                                                <a href="{{ route('genders.edit', $gender->id) }}" class="btn btn-primary btn-sm"><i data-acorn-icon="edit" data-acorn-size="13"></i> Edit</a>
                                                            @endcan --}}
                                                            {{-- @can('master-delete')
