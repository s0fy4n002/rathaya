...
@section('panel')
    <p class="font-semibold text-2xl">{{ __('fem01pic.titleU') }}</p>
    <form id="frmData" class="tooltip-label-end" action="{{ route('member_profile_update') }}" method="post" enctype="multipart/form-data">
        @csrf
        @method("PUT")
        <div class="mt-4 flex flex-col gap-4">
            ...
            <div class="form-panel">
                ...
                <div class="input-group">
                    <input type="number" class="form-akm @error('idnumber') is-invalid @enderror" value="{{ $data->idnumber }}" name="idnumber">
                ...
                <div class="input-group">
                    <select type="text" name="gender_id" class="form-akm select2 @error('gender_id') is-invalid @enderror">
                        ...
                        @foreach($genders as $gender)
                            <option value="{{$gender->id}}" {{ ( $data->gender_id == $gender->id) ? 'selected' : '' }} {{ ( $gender->f_active <> 1) ? 'disabled' : '' }}>{{$gender->id}} - {{ Config::get('app.locale') == "id" ? $gender->name_lid : $gender->name_len }}</option>
                        @endforeach
                    </select>
                    @error('gender_id')
                        <small class="text-red-500">{{ $message }}</small>
            ...
            <div class="form-panel">
                ...
                <div class="input-group">
                    <textarea type="text" class="form-akm @error('address') is-invalid @enderror" name="address">{{ $data->address }}</textarea>
                    @error('address')
                        <small class="text-red-500">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="input-group">
                    <select type="text" name="district_id" id="cboKecamatan" class="form-akm select2 @error('district_id') is-invalid @enderror">
                        ...
                        @foreach($districts as $district)
                            <option value="{{$district->id}}" {{ ( $data->district_id == $district->id) ? 'selected' : '' }}>{{$district->id}} - {{$district->name}}</option>
                        @endforeach
                    </select>
                    @error('district_id')
                        <small class="text-red-500">{{ $message }}</small>
            ...
            <div class="form-panel">
                ...
                <div class="input-group">
                    <input type="text" class="form-akm @error('villagename') is-invalid @enderror" value="{{ $data->villagename }}" name="villagename">
                    @error('villagename')
                        <small class="text-red-500">{{ $message }}</small>
                    @enderror
                </div>
            ...
            <div class="form-panel">
                ...
                <div class="input-group">
                    <input type="file" class="form-akm" name="idphoto">
                    <small class="italic">Maksimal 1MB bentuk bujur sangkar.</small>
                </div>
            </div>
        </div>
