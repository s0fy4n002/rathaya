                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Edit Slider</h1>
                        ...
                        <section class="scroll-section" id="basic">
                            ...
                            <div class="card mb-5">
                                ...
                                {{-- tooltip-label-end inputs should be wrapped in form-group class --}}
                                    <form id="frmPermission" class="tooltip-label-end" action="{{ route('sliders.update', $data->id) }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        @method("PUT")
                                        ...
                                        <div class="mb-3 position-relative form-group">
                                            <label class="form-label">Button (ID)*</label>
                                                <textarea class="form-control" name="button_lid" rows="3" cols="50" required>{{ $data->button_lid }} </textarea>
                                                <div class="form-text">Input <strong>minimum 5 characters.</strong></div>
                                                @error('button_lid')<div class="form-text text-danger">{{ $message }}</div>@enderror
                                        </div>
                                        <div class="mb-3 position-relative form-group">
                                            <label class="form-label">Button (EN)*</label>
                                                <textarea class="form-control" name="button_len" rows="3" cols="50" required>{{ $data->button_len }} </textarea>
                                                <div class="form-text">Input <strong>minimum 5 characters.</strong></div>
                                                @error('button_len')<div class="form-text text-danger">{{ $message }}</div>@enderror
                                        </div>
                                        <div class="mb-3 position-relative form-group">
                                            <label class="form-label">Image</label>
                                            <input type="file" class="form-control" name="image" accept="image/jpg, image/jpeg" />
                                            <div class="form-text">File : .jpg / .jpeg with dimension wide 4:3, and max size 1MB. <strong>Tool resize : <a href="https://redketchup.io/image-resizer" target="_blank">click here</a></strong></div>
                                            @error('image')<div class="form-text text-danger">{{ $message }} Tool resize : <strong><a href="https://redketchup.io/image-resizer" target="_blank">click here</a></strong>.</div>@enderror
                                            <img src="/imgfe/sliders/{{ $data->image <> '' ? $data->image : 'empty.jpg' }}" class="img-fluid rounded img-fluid rounded mb-1 me-1 sw-35" alt="logo image" />
                                        </div>
                                        <div class="mb-3 position-relative form-group">
