                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Role Lists</h1>
                            ...
                            <div class="col-12 col-md-5 d-flex align-items-start justify-content-end">{{-- Top Buttons Start --}}
                                ...
                                <button type="button" class="btn btn-outline-primary btn-icon btn-icon-start w-100 w-md-auto add-datatable" onclick="window.location.href='{{ route('roles.create') }}';">
                                    ...
                                    <table
                                        ...
                                        <tbody>
                                            @forelse ($roles as $role)
                                                <tr>
                                                    <td>{{ $role->id }}</td>
                                                    <td class="text-alternate">{{ $role->name }}</td>
                                                    <td class="text-alternate">
                                                        <form action="{{ route('roles.destroy', $role->id) }}" method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            <a href="{{ route('roles.show', $role->id) }}" class="btn btn-warning btn-sm">{{-- <idata-acorn-icon="eye"data-acorn-size="13"></i> --}} Show</a>
                                                            @can('role-edit')
                                                                <a href="{{ route('roles.edit', $role->id) }}" class="btn btn-primary btn-sm">{{-- <idata-acorn-icon="edit"data-acorn-size="13"></i> --}} Edit</a>
                                                            @endcan
                                                            {{-- @can('role-delete')
                                                                @if ($role->name!=Auth::user()->hasRole($role->name))
                                                                    <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Do you want to delete this role?');"><i data-acorn-icon="bin" data-acorn-size="13"></i> Delete</button>
                                                                @endif
                                                            @endcan --}}
                                                        </form>
                                                    </td>
