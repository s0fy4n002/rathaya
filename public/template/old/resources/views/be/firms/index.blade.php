                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Business List</h1>
                            ...
                            <div class="col-12 col-md-5 d-flex align-items-start justify-content-end">{{-- Top Buttons Start --}}
                                ...
                                <button type="button" class="btn btn-outline-primary btn-icon btn-icon-start w-100 w-md-auto add-datatable" onclick="window.location.href='{{ route('firms.create') }}';">
                                    ...
                                    <table
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-muted text-small text-uppercase">Business Entity</th>
                                                <th class="text-muted text-small text-uppercase">WA Number</th>
                                                <th class="text-muted text-small text-uppercase">Province</th>
                                                <th class="text-muted text-small text-uppercase">PIC</th>
                                                <th class="text-muted text-small text-uppercase">Verification</th>
                                        ...
                                        <tbody>
                                            @forelse ($datas as $data)
                                                <tr>
                                                    <td>{{ $data->id }}</td>
                                                    <td class="text-alternate">{{ $data->name }}</td>
                                                    <td class="text-alternate">{{ $data->bentity->nameshort_len }}</td>
                                                    <td class="text-alternate">{{ $data->wanumber }}</td>
                                                    <td class="text-alternate">{{ $data->province_id }} - {{ $data->province->name }}</td>
                                                    <td class="text-alternate">{{ $data->pic->name }}</td>
                                                    <td class="text-alternate">{!! $data->verification_id == "1" ? '<span class="badge bg-success"> Yes </span>' : "" !!}</td>
                                                    <td class="text-alternate">
                                                        <form action="{{ route('firms.destroy', $data->id) }}" method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            <a href="{{ route('firms.show', $data->id) }}" class="btn btn-warning btn-sm"><i data-acorn-icon="eye"data-acorn-size="13"></i> Show</a>
                                                            @can('master-edit')
                                                                <a href="{{ route('firms.edit', $data->id) }}" class="btn btn-primary btn-sm"><i data-acorn-icon="edit" data-acorn-size="13"></i> Edit</a>
                                                            @endcan
                                                            {{-- @can('master-delete') --}}
                                                                {{-- <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Do you want to delete this gender?');"><i data-acorn-icon="bin" data-acorn-size="13"></i> Delete</button> --}}
                                                            {{-- @endcan --}}
                                                        </form>
                                                    </td>
                                                </tr>
