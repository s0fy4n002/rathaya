                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Edit District</h1>
                        ...
                        <section class="scroll-section" id="basic">
                            ...
                            <div class="card mb-5">
                                ...
                                {{-- tooltip-label-end inputs should be wrapped in form-group class --}}
                                    <form id="frmPermission" class="tooltip-label-end" action="{{ route('districts.update', $district->id) }}" method="post">
                                        @csrf
                                        @method("PUT")
                                        <div class="mb-3 position-relative form-group w-100">
                                            ...
                                            <select id="select2Regencies" name="regency_id" required>
                                                ...
                                                @foreach($regencies as $regency)
                                                    <option value="{{$regency->id}}" {{ ( $regency->id == $district->regency_id) ? 'selected' : '' }}>{{$regency->id}} - {{$regency->name}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="mb-3 position-relative form-group">
                                            ...
                                            <input type="text" class="form-control" name="id" value="{{ $district->id }}" required />
                                        ...
                                        <div class="mb-3 position-relative form-group">
                                            ...
                                            <input type="text" class="form-control" name="nameupper" value="{{ $district->nameupper }}" required />
                                            <div class="form-text">Input <strong>minimum 5 characters.</strong></div>
                                            @error('nameupper')<div class="form-text">{{ $message }}</div>@enderror
                                        </div>
                                        <button type="submit" class="btn btn-icon btn-icon-end btn-primary"> <span>Update District</span><i data-acorn-icon="chevron-right"></i></button>
                                    </form>
                                </div>
                            </div>
                        </section>
