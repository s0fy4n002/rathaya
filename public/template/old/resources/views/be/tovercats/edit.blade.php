                    ...
                    <div class="page-title-container">{{-- Title and Top Buttons Start --}}
                        <div class="row">
                            <div class="col-12 col-md-7">{{-- Title Start --}}
                                <h1 class="mb-0 pb-0 display-4" id="title">Edit Turnover</h1>
                        ...
                        <section class="scroll-section" id="basic">
                            ...
                            <div class="card mb-5">
                                ...
                                {{-- tooltip-label-end inputs should be wrapped in form-group class --}}
                                    <form id="frmPermission" class="tooltip-label-end" action="{{ route('tovercats.update', $data->id) }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        @method("PUT")
                                        <div class="mb-3 position-relative form-group">
                                            <label class="form-label">Name (ID)*</label>
                                            <input type="text" class="form-control" name="name_lid" value="{{ $data->name_lid }}" required />
                                        ...
                                        <div class="mb-3 position-relative form-group">
                                            ...
                                            <input type="text" class="form-control" name="cat_lid" value="{{ $data->cat_lid }}" required />
                                            <div class="form-text">Input <strong>minimum 5 characters.</strong></div>
                                            @error('cat_lid')<div class="form-text text-danger">{{ $message }}</div>@enderror
                                        </div>
                                        <div class="mb-3 position-relative form-group">
                                            <label class="form-label">Name (EN)*</label>
                                            <input type="text" class="form-control" name="name_len" value="{{ $data->name_len }}" required />
                                            <div class="form-text">Input <strong>minimum 5 characters.</strong></div>
                                            @error('name_len')<div class="form-text text-danger">{{ $message }}</div>@enderror
                                        </div>
                                        <div class="mb-3 position-relative form-group">
                                            <label class="form-label">Category (EN)*</label>
                                            <input type="text" class="form-control" name="cat_len" value="{{ $data->cat_len }}" required />
                                            <div class="form-text">Input <strong>minimum 5 characters.</strong></div>
                                            @error('cat_len')<div class="form-text text-danger">{{ $message }}</div>@enderror
                                        </div>
                                        <div class="mb-3 position-relative form-group">
